-- Extend submit_public_complaint to accept optional priority_level (defaults via trigger).
-- Rollback: re-apply 20260602130000_submit_public_complaint_rpc.sql
...
BEGIN
  ...
  v_priority_level := upper(NULLIF(trim(COALESCE(p_payload->>'priority_level', '')), ''));
  IF v_priority_level IS NULL OR v_priority_level NOT IN ('LOW', 'MEDIUM', 'HIGH') THEN
    IF COALESCE((p_payload->>'priority')::boolean, false) THEN
...
COMMENT ON FUNCTION public.submit_public_complaint(jsonb) IS
  'Atomically create PUBLIC_QR ticket (with priority_level), submission row, and consume OTP session.';
