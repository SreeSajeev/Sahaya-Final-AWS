...
export default function Analytics({ clientReportsMode = false }: AnalyticsProps) {
  ...
  const handleExportTickets = useCallback(() => {
    ...
    const baseHeaders = ['ticket_number', 'status', 'complaint_id', 'vehicle_number', 'vehicle_name', 'vehicle_type', 'category', 'incident_title', 'issue_type', 'state', 'location', 'opened_at', 'created_at', 'priority', 'priority_level', 'confidence_score', 'needs_review', 'client_slug', 'resolution_category'];
    const headerDisplay = [
