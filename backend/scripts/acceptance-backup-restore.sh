...
echo "===== DUMP ====="
docker exec sahaya-migration-db pg_dump -U postgres -d sahaya -Fc -f /tmp/sahaya-accept.dump
...
docker run -d --name "$TMP_NAME" -e POSTGRES_PASSWORD=accept_tmp_only -e POSTGRES_DB=sahaya_restore "$IMG"
# wait ready
for i in $(seq 1 30); do
  if docker exec "$TMP_NAME" pg_isready -U postgres >/dev/null 2>&1; then break; fi
  sleep 2
done
docker cp "$DUMP" "$TMP_NAME:/tmp/restore.dump"
docker exec "$TMP_NAME" pg_restore -U postgres -d sahaya_restore --no-owner --no-acl /tmp/restore.dump || {
  # some dumps need create; try create db already exists
  echo "pg_restore exited $? — checking counts anyway"
}

echo "===== COMPARE COUNTS ====="
live_count() { docker exec sahaya-migration-db psql -U postgres -d sahaya -tAc "$1"; }
rest_count() { docker exec "$TMP_NAME" psql -U postgres -d sahaya_restore -tAc "$1"; }
