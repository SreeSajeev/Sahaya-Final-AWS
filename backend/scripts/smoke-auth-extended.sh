...
for ROLE in SUPER_ADMIN ADMIN STAFF FIELD_EXECUTIVE; do
  ...
  RESP="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -X POST ${API_BASE}/auth/login \
  ...
  [ -n "$TOKEN" ] || continue
  curl -sS ${API_BASE}/auth/me -H "Authorization: Bearer $TOKEN" \
    | python3 -c 'import sys,json; p=(json.load(sys.stdin).get("profile") or {}); print("me", p.get("role"), bool(p.get("id")))'
  code="$(curl -sS -o /tmp/tix.json -w "%{http_code}" "${API_BASE}/data/tickets?limit=5" -H "Authorization: Bearer $TOKEN")"
...
REF1="$(curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -X POST ${API_BASE}/auth/refresh -H 'Content-Type: application/json' -d '{}')"
...
if [ -n "${OLD_REFRESH}" ]; then
  OLD="$(curl -sS -X POST ${API_BASE}/auth/refresh -H 'Content-Type: application/json' -d "{\"refreshToken\":\"${OLD_REFRESH}\"}")"
...
curl -sS -c "$COOKIE_JAR" -b "$COOKIE_JAR" -X POST ${API_BASE}/auth/logout -H 'Content-Type: application/json' -d '{}' >/tmp/logout.json
...
ADM="$(curl -sS -X POST ${API_BASE}/auth/login -H 'Content-Type: application/json' -d "{\"email\":\"${ADM_EMAIL}\",\"password\":\"${AUTH_SET_PASSWORD}\"}")"
...
curl -sS -o /tmp/adm_tix.json -w "%{http_code}" "${API_BASE}/data/tickets?limit=50" -H "Authorization: Bearer $ADM_TOKEN" >/tmp/adm_code.txt
python3 - <<PY
import json
adm_org = """$ADM_ORG"""
...
const url = `${API_BASE}/data/tickets/${chosen.ticket_id}/comments/${chosen.id}/proofs/0/url`;
